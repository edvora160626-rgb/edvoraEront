...
import { createDepartment, DEPARTMENT_STATUSES } from "../../utils/departmentApi";
...
const inputClass =
  "w-full h-[42px] rounded-lg border border-[#D0D5DD] bg-white px-3 text-[14px] text-[#344054] outline-none focus:border-[#7F56D9]";
...
function AddDepartmentModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full max-w-[720px] h-[100dvh] sm:h-auto sm:max-h-[90vh] bg-white rounded-none sm:rounded-[14px] shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <div className="h-14 sm:h-16 px-4 sm:px-6 flex items-center justify-between border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#F3E8FF] text-[#7F56D9]">
              <Building2 size={18} />
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <Hash
                  size={16}
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-[#7F56D9]"
                />
                <input
                  type="text"
                  name="departmentCode"
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full rounded-lg border border-[#D0D5DD] bg-white p-3 text-[14px] text-[#344054] outline-none resize-none focus:border-[#7F56D9]"
              />
            </div>
          </div>
        </div>
        ...
        <div className="px-4 sm:px-6 py-4 border-t border-gray-200 flex justify-end gap-3 shrink-0">
          ...
          <button
            ...
            disabled={submitting}
            className="px-6 h-[42px] rounded-lg bg-[#7F56D9] hover:bg-[#6941C6] text-white text-sm font-semibold disabled:opacity-60 disabled:cursor-not-allowed"
          >
    ...
    <div>
      <div className="mb-6 sm:mb-8 flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 h-[42px] rounded-lg bg-[#7F56D9] hover:bg-[#6941C6] text-white text-sm font-semibold shadow-sm"
        >
          <Plus size={18} />
          Add Department
        </button>
      ...
      {departments.length === 0 ? (
        <div className="bg-white rounded-xl shadow p-10 text-center border border-slate-100">
          <span className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#F3E8FF] text-[#7F56D9]">
            <Building2 size={22} />
          </span>
          <p className="text-slate-700 font-medium">No departments yet</p>
